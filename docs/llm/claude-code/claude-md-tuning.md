...
| 대상 | 경계 |
| --- | --- |
| "지시 200개" | **출처 없음.** 공식의 "200줄"과 **단위가 다릅니다** |
| "500줄" | 근거 없는 경험칙. 공식은 200줄 |
| 자동 생성 금지 | 공식은 `/init`을 권합니다. **초안으로만 쓰면 양립** |
| 스타일 규칙 배제 | 린터가 강제할 수 있는 것에만 해당 |
| `file:line` 포인터 | **줄 번호는 코드 조각보다 빨리, 더 조용히 썩습니다** |
| `.claude/` 맥락 파일 | 로드가 **모델 판단**에 달림. `paths` 규칙이 더 확실 |
| 이 글 전체 | 자사 제품(Tembo) 홍보를 겸합니다 |
