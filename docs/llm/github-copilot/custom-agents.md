...
| 대상 | 경계 |
| --- | --- |
| 이 글의 권위 근거 | **2,500개 분석의 방법론·데이터·성공 기준이 비공개**입니다 |
| 3단 경계 | 마크다운의 "Never do"는 **권고**입니다. 강제는 `tools` 목록 쪽 |
| 본문 길이 | 30,000자 한도(공식 문서). **권장 길이는 어느 쪽에도 없습니다** |
| 원문의 "여섯 에이전트" | 본문에는 다섯 개만 있습니다 |
| `mcp-servers`·`metadata` | **GitHub.com 전용**입니다 |
| 이 문서 전체 | Copilot을 직접 쓰지 않았습니다. **전부 읽은 것입니다** |
