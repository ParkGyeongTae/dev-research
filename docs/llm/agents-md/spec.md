...
## 여기 적은 명령은 문서가 아니라 실행 대상입니다

FAQ에 있는 문장이 이 페이지에서 가장 실무적인 내용입니다.
...
## 그래서 이 포맷이 보장하는 것은 무엇인가

여기까지를 한 줄로 줄이면 이렇습니다. **AGENTS.md가 보장하는 것은 "루트에 이 이름의 마크다운 파일이 있다"까지이고, 그 밖은 전부 도구의 구현입니다.**

앞 절들에서 하나씩 확인한 그대로입니다. 포맷 검증은 **없습니다** — 필수 필드도 스키마도 린터도 없습니다(맨 위 FAQ). 준수 보장도 없습니다 — 사이트 스스로 "사용자 채팅 프롬프트가 모든 것을 덮는다"고 씁니다. 병합·탐색 방식은 사이트가 "가장 가까운 것이 이긴다"고 하지만 **Claude Code는 이어붙입니다.** 지원 목록의 23개 도구도 일부는 **기본 동작이 아니라 설정이 필요합니다.** 그리고 적은 명령은 문서가 아니라 실행 대상입니다.

여기에 사이트가 아예 다루지 않는 공백이 하나 더 있습니다. **크기에 대한 기준이 없습니다.** 길이 권고도, 지시 개수 한계도, 파일이 커졌을 때 어떤 증상이 나오는지도 전혀 적혀 있지 않습니다. 무엇을 담을지는 말하지만 **얼마나 담으면 망가지는지는 말하지 않습니다** — 이 사이트를 읽고 나서 스스로 채워야 하는 부분입니다.
