...
다른 대분류가 **도메인**(어디에 쓰이는가) 기준인 것과 달리, 여기는 **재료**(무엇으로 쓰는가) 기준입니다. 그래서 갈림길을 분명히 해 둡니다.
...
- **셸(sh·bash·dash·zsh)은 언어이자 대화형 환경입니다.** 문서의 중심은 **언어로서의 셸**입니다 — 문법, POSIX 이식성, 스크립트가 깨지는 지점. 대화형 환경 자체를 일반론으로 다루지는 않습니다.
  - **셸 계열은 `languages/shell/` 한 폴더에 모읍니다.** 셸별로 폴더를 나누지 않는 이유는 `sh`에 있습니다 — `sh`는 구현체가 아니라 **규격 이름**이고, 실제로 그 자리에 걸리는 것은 배포판에 따라 bash·dash 등으로 달라집니다. 구현체와 규격을 같은 층위의 폴더로 세우면 구조가 문서 내용과 어긋납니다.
  - **특정 셸의 생태계 도구**(Oh My Zsh 등)도 같은 폴더에 둡니다 — npm·nvm이 `backend/nodejs/` 폴더 안에 있는 것과 같은 방식입니다.
