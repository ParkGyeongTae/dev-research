...
2. **규격을 어떻게 읽는가** — `shall`/`should`/`may`, 그리고 "정하지 않았다"의 세 종류 (§4~§5)
3. "POSIX 준수"라는 말을 어디까지 믿을 수 있는가 (§6~§8)
...
| POSIX를 지키면 어디서나 같은 결과가 나온다 | 하한선의 보장입니다. 같은 동작의 보장이 아닙니다(§4) |
...
- 네 번째 권은 **정보성(informative)** 입니다. 읽으면 이해에는 크게 도움이 되지만, 거기 적힌 것을 근거로 "규격이 이렇게 정했다"고 말할 수 없습니다.

두 번째가 특히 자주 헷갈립니다. **규범(normative)과 정보성의 구분은 Rationale 권에서 끝나지 않고 규격 안에서 계속 나옵니다** — 개별 유틸리티 페이지의 APPLICATION USAGE·EXAMPLES 절도 정보성입니다. 그래서 "`sh` 페이지에 이렇게 쓰여 있다"가 곧 "규격이 그렇게 정했다"는 뜻이 아닙니다. §7의 `#!`가 정확히 이 함정에 걸리는 사례입니다.
...
이 세 단어가 있다는 사실 자체가 "POSIX 준수 = 어디서나 같은 동작"이라는 기대를 무너뜨립니다. **준수는 `shall` 부분에 대한 하한선이고, `should`·`may` 위쪽은 구현이 각자 정합니다.** 그래서 환경을 옮겼을 때 오류가 아니라 **값만 달라지는** 형태의 차이가 생깁니다.
...
여기서 나오는 실무적 결론이 있습니다. **"규격에 없다"를 "어디서도 안 된다"로 읽으면 안 되고, "여러 구현에서 다 된다"를 "이식성 있다"로 읽어도 안 됩니다.** 이 두 오독이 만나면 `local`처럼 널리 도는 것을 안전하다고 판단하게 되고, 그 판단은 다른 `sh` 구현으로 옮길 때 깨집니다.
...
이것이 규격을 잘못 읽었을 때 나오는 최악의 형태입니다. **요란하게 죽는 것보다, 오류 없이 값만 달라지는 쪽이 훨씬 오래 살아남습니다.** "POSIX에 있으니까 괜찮다"고 판단한 문법이 실제로는 이 시스템의 판보다 나중에 들어온 것이었고, 그 사실은 아무 오류로도 드러나지 않습니다.
...
그러므로 인증 여부는 **이식성 판단의 근거가 되지 못합니다.** "맥이 인증받았으니 맥에서 돌면 이식성 있다"도, "리눅스는 인증이 없으니 덜 표준적이다"도 둘 다 틀린 추론입니다.
...
두 번째 명령은 아무것도 출력하지 않았습니다 — 비대화형이므로 `ENV`를 읽지 않은 것입니다. §4(a)에서 본 `shall`이 실제로 관측되는 모습이 이것입니다.

## 8. 그래서 언제 POSIX를 기준으로 삼는가

여기까지가 규격을 읽는 법이라면, 마지막은 **그 기준을 언제 쓸 것인가**입니다. POSIX 이식성은 공짜가 아니기 때문입니다.

**이식성은 "여러 환경에서 돌 때"만 값이 있습니다.** 컨테이너 이미지를 못 박아 두고 그 안에서만 도는 스크립트라면, 규격 최소 집합에 맞추느라 `printf` 곡예를 하는 것보다 `#!/bin/bash`를 명시하고 bash 기능을 쓰는 편이 읽기 쉽고 안전합니다. 반대로 배포 대상이 여럿이거나 무엇이 걸릴지 모르는 자리(Dockerfile의 `RUN`, CI 스텝)라면 규격이 기준선 역할을 합니다.

기준으로 삼기 어려운 경우도 분명합니다.

- **배열·연관 배열·정규식 매칭이 필요한 로직.** 규격의 셸 언어에는 이 셋이 없습니다. 문자열 조작으로 흉내 내기 시작했다면 셸의 경계를 넘은 신호입니다 — Python으로 옮기는 편이 낫습니다.
- **GNU 유틸리티 옵션에 기대고 있을 때.** `sed -i`, `date -d`, `grep -P`, `readlink -f`는 모두 규격 밖입니다. 이건 셸이 아니라 **유틸리티 쪽** 문제라 셸을 dash로 맞춰도 해결되지 않습니다.
- **"POSIX 준수"를 품질 근거로 쓸 때.** §5에서 봤듯 준수 판번호는 시스템마다 다르고, §4·§7에서 봤듯 규격이 정하지 않고 남겨 둔 영역이 넓습니다. 준수는 하한선이지 품질 등급이 아닙니다.
