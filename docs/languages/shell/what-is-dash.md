...
이 문서가 잡으려는 개념은 셋입니다. dash가 **무엇의 구현인지**, **무엇이 없는지**, 그리고 **없는 것을 건드렸을 때 어떤 모양으로 실패하는지**입니다.
마지막이 중심입니다 — dash의 실패는 요란하게 죽는 쪽보다 **조용히 잘못된 값을 내거나, 실패하고도 0을 반환하는 쪽**이 많습니다(§4).
...
| `/bin/dash` | Apple 패키지 `dash-16` — **상위 버전 번호는 확인하지 못했습니다**(§7) |
...
| Alpine | BusyBox ash — dash가 **아닙니다** | **확인 필요** (§7) |
...
같은 표현식을 네 셸에 넣고 첫 줄만 잘라 비교했습니다. **직접 실행한 결과입니다**(각 칸은 앞 22자).
...
**`${PIPESTATUS[@]}`는 어느 판에도 없습니다.** trixie man page에도 없습니다. 파이프 중간의 실패를 개별로 집어내야 한다면 dash로는 방법이 없고, 스크립트를 bash로 올리거나 파이프를 풀어 임시 파일로 끊어야 합니다. `psql ... | gzip > out.gz` 형태에서 앞 단계 실패를 알아야 한다면 dash는 맞지 않습니다.
...
| 셸 | 1회차 | 2회차 | 3회차 |
| --- | --- | --- | --- |
| `/bin/dash` | 1.13s | 1.11s | 1.12s |
| `/bin/bash` | 1.45s | 1.43s | 1.43s |
| `/bin/zsh` | 2.15s | 2.17s | 2.14s |
| `/bin/sh` (=bash) | 2.27s | 2.27s | 2.30s |
...
| `/bin/bash` | 0.21s | 0.21s | 0.22s |
| `/bin/zsh` | 0.12s | 0.11s | 0.12s |
...
3. **`/bin/sh`가 `/bin/bash`보다 느립니다(2.27s vs 1.43s).** 같은 bash 3.2.57인데도 그렇습니다. 원인을 분리해 봤습니다.

```console
$ /bin/bash -c :          → real=1.46s
$ /bin/bash --posix -c :  → real=1.43s
$ /bin/sh -c :            → real=2.25s
```
...
대화형 셸로 dash를 고를 이유도 없습니다. vi/emacs 편집 모드는 있지만(`/bin/dash -V`, `-E` 모두 수용됨 — 직접 확인) 보완·프롬프트 확장·이력 관리가 bash·zsh 수준이 아닙니다.
