...
# `.bashrc`란 무엇인가 — 로그인 셸은 이 파일을 읽지 않습니다

`.bashrc`에 대한 가장 흔한 오해는 이것입니다. **"셸을 켜면 읽히는 파일."**

아닙니다. bash가 `~/.bashrc`를 읽는 조건은 **"대화형이면서 로그인 셸이 아닐 때"** 하나뿐입니다.
그래서 SSH로 접속하거나 `bash --login`으로 띄우면 `.bashrc`는 **읽히지 않습니다.** 많은 사람의 `.bash_profile` 맨 끝에 `. ~/.bashrc` 한 줄이 붙어 있는 이유가 이것입니다 — 그 줄이 없으면 두 세계가 갈라집니다.

이 문서는 세 갈래(로그인 / 대화형 비로그인 / 비대화형)를 **각각 돌려서** 어떤 파일이 실제로 읽히는지 확인합니다.
...
`~/.bashrc`·`~/.bash_profile`은 **bash가 자기 재량으로 정한 파일 이름**입니다. 그래서 "셸 설정 파일"이라는 일반론으로 다룰 수 없고, bash 매뉴얼이 유일한 기준입니다.
...
**bash 5.3 매뉴얼에서도 같은 규칙입니다.** 현행 매뉴얼(2025-04-07 판)의 같은 절을 대조했고, 파일 이름·순서·조건이 모두 동일했습니다.
— [Chet Ramey, BASH(1) manual page (2025 April 7)](https://tiswww.case.edu/php/chet/bash/bash.html) (확인: 2026-09-06)

즉 **이 규칙은 20년 가까이 바뀌지 않았습니다.** 버전 차이로 설명할 수 있는 문제가 아니라는 뜻입니다.
...
이번엔 반대입니다. `.bashrc`만 읽히고 `.bash_profile`은 읽히지 않았습니다.
...
**아무 파일도 읽히지 않았습니다.** 이것이 cron·CI·`ssh host '명령'`에서 "PATH에 아무것도 없다"가 나오는 이유입니다.
...
## 4. `.bash_profile` · `.bash_login` · `.profile` — 셋 중 하나만 읽힙니다
...
`.bashrc`가 있는데도 읽지 않았습니다. `ENV`를 주면 그때만 읽습니다.
...
`/bin/sh`는 bash 3.2가 `sh` 이름으로 도는 것이고, 따라서 **`/bin/sh` 스크립트는 `.bashrc`와 아무 상관이 없습니다.**
...
- **`/etc/bashrc`는 bash가 자동으로 읽는 파일이 아닙니다.** `/etc/profile`이 명시적으로 `source` 하기 때문에 읽히는 것입니다. 즉 **로그인 셸 경로에서만** 걸립니다.
- 맨 앞의 `if [ -z "$PS1" ]; then return; fi`가 **비대화형이면 즉시 반환**하는 관용구입니다. 개인 `.bashrc`에서도 흔히 쓰이는 패턴이고, §7의 첫 번째 실패 모드가 여기서 나옵니다.

`/etc/bash.bashrc`는 이 맥에 **없습니다**(리눅스 배포판 쪽 관례입니다).

## 7. 실패 모드

### (a) 로그인 셸에서만 alias·함수가 사라진다

`.bashrc`에 alias를 넣고 `.bash_profile`에서 `source` 하지 않은 상태입니다.
...
### (b) 스크립트·cron에서 PATH가 비어 있다

§3(c)에서 본 그대로입니다. 비대화형 bash는 **어떤 개인 설정 파일도 읽지 않습니다.**
`.bashrc`에 `export PATH=...`를 넣어 두고 대화형 터미널에서만 검증하면, cron이나 CI에서 `command not found`가 납니다.

- 고치는 방향은 **파일을 옮기는 것이 아니라 의존을 없애는 것**입니다. 스크립트 안에서 절대 경로를 쓰거나, PATH를 스크립트 자신이 설정하게 합니다.
- `BASH_ENV`로 억지로 읽히게 만들 수는 있지만(§3), 그러면 **모든 비대화형 bash가 그 파일을 읽게 되어** 스크립트 실행이 느려지고 예상 못 한 부작용이 생깁니다. 권하지 않습니다. (의견)

### (c) `.bashrc`의 출력이 `scp`·`rsync`를 깨뜨린다

`.bashrc`에 `echo`·배너·`fortune` 같은 **표준 출력에 무언가를 찍는 코드**가 있으면, 그 출력이 원격 명령의 데이터 스트림에 섞여 `scp`·`sftp`·`rsync`가 실패할 수 있습니다.

bash 매뉴얼이 원격 실행 시 `.bashrc`를 읽는 경로를 따로 정의해 두고 있습니다.
...
**이 시나리오는 재현하지 못했습니다**(§9). SSH 서버가 필요한 실험이라 이 머신에서 돌리지 못했습니다. 위 문장은 매뉴얼에서 읽은 것이고, 실제로 `sshd` 경유에서 어떻게 동작하는지는 빌드 옵션과 배포판에 따라 갈립니다. **확인 필요.**

다만 대응책은 조건과 무관하게 유효합니다 — **`.bashrc`에서 표준 출력에 아무것도 찍지 않는 것**입니다. 꼭 찍어야 하면 대화형일 때로 한정합니다.
...
| `~/.bash_profile` 맨 끝 | `[ -r ~/.bashrc ] && . ~/.bashrc` | 로그인 셸에서도 대화형 설정이 살아 있게 만듭니다 |

핵심 구분은 **상속되는가**입니다. 환경 변수는 `export` 하면 자식이 물려받으므로 로그인 때 한 번이면 충분합니다. alias·함수는 물려받지 않으므로 셸이 뜰 때마다 다시 정의돼야 합니다.

## 9. 경계 — 이 문서가 안 맞는 곳

- **맥의 기본 로그인 셸은 zsh입니다.** 이 맥에서 `dscl . -read /Users/$USER UserShell`은 `/bin/zsh`를 돌려줍니다. 터미널을 열어 나오는 셸이 bash가 아니라면 `.bashrc`는 아무 영향도 주지 않습니다 — 파일을 아무리 고쳐도 변화가 없는 것이 정상입니다.
- **`#!/bin/sh` 스크립트에는 적용되지 않습니다.** §5에서 봤듯 `sh` 이름으로 도는 bash는 `.bashrc`를 읽지 않습니다.
- **컨테이너 안에서는 대개 무의미합니다.** `docker run` 기본 실행은 비대화형이고, `RUN` 명령도 마찬가지입니다. 이미지 안에서 PATH를 잡고 싶으면 Dockerfile의 `ENV`를 쓰는 것이 맞습니다.
- **맥의 bash가 3.2라는 점.** 시작 파일 규칙은 5.3과 같지만(§2), `.bashrc`에 넣는 **내용**은 다릅니다. 연관 배열(`declare -A`)이나 `${var^^}` 같은 4.0 이후 문법을 넣으면 맥의 `/bin/bash`에서 문법 오류가 납니다.

## 10. 확인하지 못한 것

- **`sshd` 경유 비대화형 실행에서 `.bashrc`가 읽히는지.** §7(c). SSH 서버 실험을 하지 못했습니다. **미실행.**
- **리눅스 배포판의 기본 `~/.bashrc`·`/etc/bash.bashrc` 내용.** 이 문서의 전역 파일 확인(§6)은 macOS 15.7.4 것뿐입니다. Debian 계열의 `skel` 파일은 원문을 받아보지 못했습니다(요청한 두 URL이 각각 503·404). **확인 필요.**
...
- 로컬 `man bash` — GNU Bash-3.2 (2006-09-28 판), macOS 15.7.4에서 확인 (2026-09-06)
- [Chet Ramey, BASH(1) manual page (2025 April 7) — bash 5.3](https://tiswww.case.edu/php/chet/bash/bash.html) (확인: 2026-09-06)
- [The Open Group Base Specifications Issue 8 / IEEE Std 1003.1-2024, `sh`](https://pubs.opengroup.org/onlinepubs/9799919799/utilities/sh.html) (확인: 2026-09-06)
- 로컬 `/etc/profile`, `/etc/bashrc` — macOS 15.7.4 (24G517), 2026-09-06 확인
- 위 실행 기록 전부 — macOS 15.7.4 (24G517), bash 3.2.57(1)-release, 2026-09-06 실행
