...
# Node.js란 무엇인가 — 데이터 엔지니어를 위한 실행 모델 정리

Node.js를 "JavaScript를 서버에서 돌리는 것" 정도로 알고 넘어가면, 나중에 **왜 CPU를 쓰는 코드 한 줄이 서버 전체를 멈추는지**를 설명하지 못합니다.
이 문서는 데이터 엔지니어가 이미 아는 실행 모델(스레드 풀, executor, GIL)과 대조해 Node.js의 실행 모델을 정리하고, 그것이 깨지는 지점을 실측으로 확인합니다.
...
- **번들된 것들이 곧 버전에 묶인 동작입니다** — OpenSSL·ICU·SQLite가 Node 실행 파일 안에 들어 있습니다. 즉 "Node를 올렸더니 TLS 동작이 바뀌었다", "정렬 순서가 바뀌었다"가 실제로 일어납니다.
...
- 또 깨지는 부분: Spark executor는 작업이 죽어도 나머지가 살지만, 이벤트 루프에서는 잡히지 않은 예외 하나가 **프로세스 전체를 죽입니다**(§6-(c)).
...
```bash
$ node block.js
동기 작업 끝: 657ms (n=3000000)
타이머(0ms 예약) 실제 실행: 660ms
```

**0ms로 예약한 타이머가 660ms 뒤에 실행됐습니다.** 그동안 이 프로세스는 어떤 HTTP 요청도 받지 못하고, 어떤 소켓 이벤트도 처리하지 못합니다.
...
- `UV_THREADPOOL_SIZE=1`에서는 **66ms씩 순서대로** 끝납니다. 정확히 직렬화됐습니다.
...
문서가 스스로 선을 그어 뒀습니다. **CPU 작업에는 쓰고, I/O 작업에는 쓰지 마라.** I/O를 워커로 감싸면 기본 비동기 I/O보다 느려집니다.

---

## 5. 경계 — Node.js가 안 맞는 곳

데이터 엔지니어 관점에서 특히 중요한 것부터 씁니다.

### (a) 64비트 정수를 다루는 파이프라인
...
**id가 조용히 바뀌었습니다.** 에러도 경고도 없습니다. Snowflake ID, Kafka offset, DB의 `BIGINT` 기본키처럼 2^53을 넘는 정수가 JSON으로 오가는 파이프라인을 Node로 중계하면 **키가 충돌하거나 유실됩니다.**
...
§3에서 본 그대로입니다. 수 GB짜리 파일을 파싱·집계·조인하는 일은 이벤트 루프를 붙잡습니다. 이건 튜닝으로 해결되는 게 아니라 **모델이 안 맞는 것**입니다. 그런 일은 Spark·DuckDB·Polars 쪽이고, Node로 굳이 하려면 `worker_threads`로 계산을 분리해야 합니다.

### (c) 수치 연산 생태계가 필요할 때

pandas·NumPy·Arrow에 해당하는 성숙한 대체재가 JavaScript 쪽엔 얇습니다. **의견:** 분석 코드를 Node로 옮길 이유는 거의 없습니다.

### (d) 반대로, Node가 잘 맞는 곳

- **I/O가 대부분이고 CPU는 거의 안 쓰는 중계 계층** — 여러 API를 팬아웃해서 합치는 게이트웨이, 웹훅 수신기, 스트림 릴레이.
- **동시 연결이 많고 각 연결이 놀고 있는 경우** — 롱폴링·SSE·WebSocket.
...
### (a) 처리 안 된 Promise 거부가 프로세스를 죽입니다
...
증상: 컨테이너가 주기적으로 재시작되는데 로그에는 스택 트레이스 한 덩어리만 남습니다.

### (b) 동기 API가 섞여 들어옵니다

`fs.readFileSync`·`execSync`처럼 이름에 `Sync`가 붙은 API는 이벤트 루프를 그 자리에서 멈춥니다. 부팅 시 설정 파일을 읽을 땐 문제없지만, **요청 처리 경로에 하나라도 들어가면** §3의 660ms가 매 요청마다 재현됩니다.

`Sync`가 안 붙었는데도 동기인 것들이 있습니다 — `zlib`의 일부 동기 변형, 큰 문자열의 `JSON.parse`/`JSON.stringify`가 대표적입니다. `JSON.parse`는 **비동기 버전이 없습니다.**

### (c) 메모리 한계가 컨테이너 한계와 다릅니다
...
증상: 로컬에서는 되는데 컨테이너에서만 OOM. 원인은 둘 중 하나이고 **로그를 봐야 구분됩니다** — V8 힙 상한에 닿았으면 Node가 `JavaScript heap out of memory`를 찍고 죽고, cgroup 제한에 걸렸으면 로그 없이 커널이 죽입니다(종료 코드 137).

---

## 출처

- Node.js 소개 — `https://nodejs.org/en/learn/getting-started/introduction-to-nodejs` (확인 2026-09-05)
- Node.js와 브라우저의 차이 — `https://nodejs.org/en/learn/getting-started/differences-between-nodejs-and-the-browser` (확인 2026-09-05)
- About Node.js (설계 의도) — `https://nodejs.org/en/about` (확인 2026-09-05)
- 이벤트 루프·타이머·nextTick — `https://nodejs.org/en/learn/asynchronous-work/event-loop-timers-and-nexttick` (확인 2026-09-05)
- libuv 스레드 풀 — `https://docs.libuv.org/en/v1.x/threadpool.html` (확인 2026-09-05)
- worker_threads — `https://nodejs.org/api/worker_threads.html` (Node v26.8.1 문서, 확인 2026-09-05)
- CLI 옵션(JSON 원본) — `https://nodejs.org/api/cli.json` (Node v26.8.1 문서, 확인 2026-09-05)
- 실행 기록 — macOS 15.7.4 / arm64 / Node v22.21.1, 2026-09-05 직접 실행
