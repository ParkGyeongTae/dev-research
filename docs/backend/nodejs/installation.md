...
# Node.js 설치 — 어떤 경로로, 어느 버전을

Node.js를 설치하는 방법은 네 가지이고, 어느 것을 고르느냐에 따라 **나중에 버전을 바꿀 수 있는지**가 갈립니다.
이 문서는 그 선택과, 선택을 잘못했을 때 나타나는 증상을 다룹니다.
...
```bash
$ curl -s https://nodejs.org/dist/index.json | node -e "let d='';process.stdin.on('data',c=>d+=c).on('end',()=>{const j=JSON.parse(d);console.log('Current:',j[0].version,j[0].date,'npm',j[0].npm);const lts=j.find(v=>v.lts);console.log('Latest LTS:',lts.version,lts.date,'npm',lts.npm,'codename',lts.lts);const l22=j.find(v=>v.lts&&v.version.startsWith('v22'));console.log('v22 latest:',l22.version,l22.date,'npm',l22.npm,l22.lts);})"
...
| 계열 | 코드네임 | 2026-09-05 기준 최신 | 번들 npm |
| --- | --- | --- | --- |
...
여기서 눈여겨볼 것은 **Node 버전마다 번들되는 npm 버전이 다르다**는 점입니다. v24/v26은 npm 11 계열, v22는 npm 10 계열입니다. Node를 바꾸면 npm도 같이 바뀝니다.
...
여기부터는 사실이 아니라 **판단**입니다.

| 상황 | 권장 | 이유 |
| --- | --- | --- |
| 개발 머신 | **버전 매니저**(nvm/fnm) | 프로젝트마다 Node 버전이 다른 게 정상이고, 전환이 안 되면 결국 하나에 맞추게 됩니다 |
| CI | **CI가 제공하는 setup 액션** 또는 컨테이너 이미지 | 셸 초기화 파일에 의존하지 않습니다 (§5-a 참고) |
| 컨테이너 | **공식 `node:<버전>` 이미지** | 이미지 태그가 곧 버전 고정입니다. 그 안에 버전 매니저를 또 넣을 이유가 없습니다 |
| 프로덕션 서버 | OS 패키지 또는 컨테이너 | 사용자별로 설치되는 버전 매니저는 서비스 계정과 어긋납니다 |

**공식 인스톨러(`.pkg`)는 개발 머신에 권하지 않습니다.** 버전을 바꾸려면 덮어쓰는 수밖에 없고, 나중에 버전 매니저를 도입할 때 남은 `/usr/local/bin/node`가 PATH에서 충돌합니다.

---

## 4. 경계 — 버전 매니저가 안 맞는 곳

- **비인터랙티브 실행**(cron, systemd, `sh -c`, 일부 CI 스텝): 셸 초기화 파일을 읽지 않으므로 nvm 자체가 로드되지 않습니다. 아래 §5-a가 실증입니다.
- **컨테이너 이미지**: 태그로 이미 버전이 고정돼 있습니다. 이미지 안에서 다시 버전을 바꿀 일이 있다면 이미지 설계가 잘못된 것입니다(의견).
- **여러 사용자가 쓰는 서버**: nvm은 정의상 사용자별 설치입니다(§5-b의 nvm 정의 인용 참고). 계정마다 따로 깔리므로 "서버에 Node를 깔았다"가 성립하지 않습니다.

---

## 5. 실패 모드

### (a) 비인터랙티브 셸에서 `node: command not found`

가장 자주 밟는 지뢰입니다. 버전 매니저는 셸 초기화 파일(`.zshrc` 등)에서 로드되는데, 비인터랙티브 셸은 그 파일을 읽지 않습니다.

같은 스크립트를 셸 모드만 바꿔 돌린 실제 결과입니다.
...
`env -i`로 환경 변수를 비운 뒤 돌린 것입니다. **인터랙티브 셸(`-i`)에서만 node가 잡힙니다.**

증상은 "로컬에서는 되는데 cron/CI에서만 `command not found`"로 나타납니다. 해결은 버전 매니저를 로드하려 애쓰는 게 아니라, 그 환경에서는 **절대 경로를 쓰거나 버전 매니저에 의존하지 않는 설치 경로를 고르는 것**입니다(의견).

### (b) 두 개의 node가 공존

로그인 셸에서 실제로 찍은 결과입니다.
...
버전 매니저를 쓰면 전역 디렉터리가 애초에 홈 아래에 잡히므로 `sudo`가 필요 없습니다. 이 환경에서 실제로 확인한 값입니다.
...
경로에 **버전 번호가 들어 있다**는 점이 중요합니다 — 전역 패키지는 Node 버전마다 따로 설치되며, Node를 바꾸면 전역으로 깔아둔 CLI가 사라진 것처럼 보입니다.
...
- Node.js 릴리스 정책 — `https://nodejs.org/en/about/previous-releases` (확인 2026-09-05)
- Node.js 다운로드 페이지 — `https://nodejs.org/en/download` (확인 2026-09-05)
- 배포 인덱스(버전·번들 npm) — `https://nodejs.org/dist/index.json` (확인 2026-09-05)
- 실행 기록 — macOS 15.7.4 / arm64 / zsh, 2026-09-05 직접 실행
