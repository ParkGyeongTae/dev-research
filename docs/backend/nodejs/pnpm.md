...
pnpm의 실제 성격은 **`node_modules`를 평평하게 만들지 않는다**는 데 있고, 디스크 절약도 유령 의존성 차단도 전부 그 한 가지 설계에서 따라 나옵니다.
...
두 번째 문장의 **"only the direct dependencies"**가 이 문서 전체의 열쇠입니다. 설치할 때 pnpm이 직접 출력하는 내용에도 이 구조가 그대로 드러납니다.
...
전역 스토어가 (1)을, virtual store와 심볼릭 링크가 (2)를 담당합니다.
...
최상위에 **선언한 `express` 하나만** 있고, 그마저도 심볼릭 링크입니다. 나머지 67개는 `.pnpm` 안에 있습니다 — §1 인용문의 "only the direct dependencies"가 그대로 관측됩니다.
...
**유령 의존성(phantom dependency)**은 `package.json`에 선언하지 않았는데 `require`가 되는 패키지입니다. §2에서 본 호이스팅의 부작용입니다.
...
**왜 이게 중요한가**: npm 쪽 코드는 로컬에서 잘 돕니다. 그러다 `express`가 하위 의존성을 정리하거나 버전을 올려 `body-parser`가 최상위로 올라오지 않게 되는 순간, **아무것도 안 고쳤는데 `MODULE_NOT_FOUND`가 납니다.** 그 시점은 대개 배포 뒤입니다.

pnpm은 그 상황을 **로컬 첫 실행에서** 터뜨립니다. 고통의 총량이 줄어드는 게 아니라 **시점이 앞당겨지는 것**입니다.

그래서 npm에서 pnpm으로 옮긴 직후 `MODULE_NOT_FOUND`가 쏟아지는 것은 정상입니다. **조치는 에러에 뜬 패키지를 `package.json`에 제대로 선언하는 것**이고, 그게 원래 있었어야 할 상태입니다. 급하면 §2의 `nodeLinker: hoisted`가 탈출구지만 **그러면 pnpm을 쓰는 이유의 절반이 사라지므로**, 임시 조치라는 것을 문서에 남겨야 합니다(의견).
...
기본값이 `auto`이고, APFS는 copy-on-write 클론을 지원하므로 pnpm이 **clone**을 골랐습니다. §1 설치 로그의 `Packages are cloned from the content-addressable store`라는 문구가 그 증거입니다.

**그래서 `du`가 절감을 못 봅니다.** 클론은 별도 inode를 가지며 블록만 공유하므로, `du`는 각 파일을 온전한 크기로 셉니다. 하드링크였다면 `du`가 한 번만 셌을 것입니다.

실무적으로 두 방향 다 틀립니다. **"pnpm 썼는데 `du` 찍어보니 그대로던데"는 잘못된 결론이고**, 반대로 **디스크 용량 산정을 `du` 기준으로 하면 실제보다 크게 잡습니다.**

> 위 결론은 **macOS APFS에서만 확인했습니다.** Linux ext4/overlayfs(=대부분의 컨테이너 환경)에서 pnpm이 하드링크를 고르는지, 그때 `du`가 절감을 보여주는지는 **확인하지 않았습니다 — 확인 필요.**

같은 이유로 **컨테이너 이미지에서 디스크 절약을 기대할 때는 주의가 필요합니다.** 스토어가 빌드 레이어 밖에 있으면 공유 효과가 사라집니다.

> 위 문장은 구조에서 따라 나오는 추론이며 **이 저장소에서 컨테이너 환경으로 재현하지 않았습니다 — 확인 필요.**
...
**메이저가 1~2년에 한 번씩 올라가고, 그때 락 파일 포맷(`lockfileVersion`)이 바뀔 수 있습니다.** 팀원과 CI가 서로 다른 pnpm 메이저를 쓰면 **락 파일이 매 커밋마다 통째로 다시 쓰이며 충돌합니다** — `pnpm-lock.yaml` 첫 줄의 `lockfileVersion`이 왔다 갔다 하면 이 경우입니다.

§6의 `packageManager` 필드로 버전을 고정하는 게 이 문제의 해법입니다(의견).

---

## 8. 그래서 언제 쓰고 언제 안 쓰는가

§2의 구조(심볼릭 링크 + virtual store)가 이점과 제약을 함께 만듭니다. 아래는 판단입니다(의견).

**쓸 만한 자리**는 §3의 엄격함이 값어치를 하는 곳입니다 — 여러 사람이 만지고 오래 사는 코드베이스, 특히 워크스페이스(모노레포)에서 패키지 간 의존 관계를 정확히 유지해야 하는 경우.

**맞지 않는 자리는 이렇습니다.**

- **심볼릭 링크를 제대로 못 다루는 도구가 파이프라인에 있을 때.** `node_modules`를 통째로 압축해 옮기거나, 링크를 따라가지 않는 배포 스크립트가 있으면 깨집니다. 이건 pnpm의 버그가 아니라 §2 구조상 당연한 결과입니다.
- **유령 의존성에 기대고 있는 의존성이 많은 레거시 프로젝트.** §3의 차단이 그대로 장애물이 됩니다. `nodeLinker: hoisted`로 우회할 수 있지만, 그러면 옮긴 의미가 줄어듭니다.
- **팀 전체가 옮기지 않을 때.** `package-lock.json`과 `pnpm-lock.yaml`이 함께 커밋되면 두 락이 서로 다른 트리를 주장합니다.
- **"빠르다"만 근거로 도입할 때.** 공식 문서는 설치 과정이 "significantly faster than the traditional three-stage installation process"라고 설명하지만(§1 출처), **이 문서에서 설치 시간을 측정하지 않았습니다.** 조건 없는 속도 비교는 인용하지 않습니다. 디스크 쪽 근거도 §4의 단서를 함께 읽어야 합니다.
